...
<%@ page import="com.liferay.portal.authenticator.ldap.configuration.LDAPAuthConfiguration" %>

<%@ include file="/init.jsp" %>

<%
LDAPAuthConfiguration ldapAuthConfiguration = ConfigurationFactoryUtil.getConfiguration(LDAPAuthConfiguration.class, 
		new ParameterMapSettingsLocator(request.getParameterMap(), new CompanyServiceSettingsLocator(company.getCompanyId(), "com.liferay.portal.authenticator.ldap.configuration.LDAPAuthConfiguration")));

System.out.println("ldapAuthConfiguration.enabled() ? " + ldapAuthConfiguration.enabled());
		 
String authenticationURL = currentURL + "#_LFR_FN_authentication";

boolean ldapAuthEnabled = ldapAuthConfiguration.enabled(); 
//PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.LDAP_AUTH_ENABLED, PropsValues.LDAP_AUTH_ENABLED); 

boolean ldapAuthRequired = ldapAuthConfiguration.required(); 
//PrefsPropsUtil.getBoolean(company.getCompanyId(), PropsKeys.LDAP_AUTH_REQUIRED); 

// NEW FIELDS?
String ldapAuthMethod = ldapAuthConfiguration.method();
String ldapAuthPasswordEncryptionAlgorithm = ldapAuthConfiguration.passwordEncryptionAlgorithm();

// OLD REDUNDANT FIELDS?
...
<aui:fieldset>
	...
	<aui:input cssClass="lfr-input-text-container" helpMessage="ldap-auth-method-help" label="ldap-auth-method" name='<%= "settings--" + PropsKeys.LDAP_AUTH_METHOD + "--" %>' type="text" value="<%= ldapAuthMethod %>" />
	
	<aui:input cssClass="lfr-input-text-container" helpMessage="ldap-auth-password-encryption-algorithm-help" label="ldap-auth-password-encryption-algorithm" name='<%= "settings--" + PropsKeys.LDAP_AUTH_PASSWORD_ENCRYPTION_ALGORITHM + "--" %>' type="text" value="<%= ldapAuthPasswordEncryptionAlgorithm %>" />
